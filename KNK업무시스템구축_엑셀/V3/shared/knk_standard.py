...
def _default_comment_for(header):
    ...
    rules = [
        ...
        # ═══════════════════════════════════════════════════════════
        # 부서 입력 파일 전용 (v3.0 / v3.1)
        # ═══════════════════════════════════════════════════════════
        # v3.1: 01 검사기 부서 파일 K11 — 본인 부서 진척률 (0/50/100)
        ("부서진척률", (
            "▣ 부서 진척률 (%) — 본인 부서 작업\n\n"
            "• 자동 계산 (수정 불가, sync 시 갱신)\n"
            "• 시작일·완료일 입력 기반:\n"
        ...
        # 키는 \n 제거 형태 (헤더가 \n 제거되어 매칭됨)
        # v3.1.1: 예정일 메모 (입고일 매칭보다 먼저 평가되어야 함)
        ("의뢰예정일", (
            "▣ 의뢰 입고예정일 (가공의뢰 후 예정)\n\n"
            "★ 날짜 입력 (% 아님 ⚠️)\n\n"
    ...
    # — 일반 형식상 "% 진척률 입력"으로 해석
    # 헤더에서 \n 제거된 형태로 표시
    h_clean = str(header).replace("\n", "")
    return (
        f"▣ {h_clean} (세부항목 진척률)\n\n"
        f"★ 진척률 % 입력 (날짜 아님 ⚠️)\n\n"
        f"• 0~1 사이 소수로 입력 (예: 0.5 = 50%, 1.0 = 100%)\n"
        f"  또는 0~100 정수도 허용 (예: 50, 100)\n\n"
        f"📋 입력값 의미:\n"
        f"  ▸ 빈 칸  → ⭕ '해당없음' (이 세부항목은 본 프로젝트 무관)\n"
        f"           — 부서 진척률 평균에서 ★분모 제외★\n"
        f"  ▸ 0     → 미착수 (분모에 포함, 0%로 계산)\n"
        f"  ▸ 0.5   → 50% 진행 중\n"
        f"  ▸ 1.0   → 100% 완료\n\n"
        f"💡 부서장·담당자가 매주 진척률 갱신\n"
        f"💡 부서진척률(%)은 해당있는 세부항목 평균으로 자동 계산"
    )
