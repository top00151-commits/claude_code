...
### 2.4.5 플로우 (flow) — 한국 AI 협업툴 (2026-04-21 추가)

**출처**: 나무위키, flow.team 공식, 머니투데이 2026.01.29, 서울신문 2026.01.29, 헤럴드경제, 마드라스체크 IR, EdrawSoft·테크뷰·Boardmix 리뷰, 다우오피스 비교
...
| 출시 | 2015년 |
| 포지셔닝 | **"대한민국 최초 AI 협업툴"** |
| 누적 사용자 | 2024년 기준 **48만 팀 · 55개국** |
| 매출 | 2024년 140억 → 2025년 **210억** (50%+ 성장) |
| 2026 목표 | 수주·계약 **300억** + **IPO 추진** |
| 글로벌 확장 | 일본·미국·영국 |
| 2024 흑자 전환 완료 | 머니투데이 2026.01.29 |

#### 🟢 장점 (사용자 언급)

- **메신저 + 업무관리 올인원** (통합 UX)
- 프로젝트 협업·캘린더·드라이브·메신저·화상회의·OKR 통합
- **flow AI 에이전트** — 목적/기획 문서 입력 → AI가 프로젝트 구조·일정·업무 리스트 자동 생성 (Gantt 자동)
- ChatGPT·Gemini·Claude·Nanobana 등 **복수 AI 모델 통합**
- 기존 툴(회의록 요약 중심)과 차별화 — **프로젝트 구조 자체 설계**
- "업무·대화·파일 흐름 이해 → 다음 실행 과제 제안" **AI Agent 기반 협업 OS** 지향
- 국내 환경 최적화
- **정부지원 최대 80% 할인** (중소기업)
...
| # | 불만 | 출처 |
|---|---|---|
| 1 | **"기능이 많다 보니 안정성을 비롯해 사용자 경험이 나쁘다"** | EdrawSoft 리뷰 |
| 2 | **"이메일 기능 제공 안 함"** — 기업 메일 필요 시 별도 서비스 신청 필수 | 다수 |
| 3 | **"무료 버전 기능이 카카오워크·네이버웍스·스윗 대비 아쉽다"** | 테크뷰 |
| 4 | **"체감할 수 있을 만큼 속도가 느리다"** | 디딤365 |
| 5 | **"메신저 형태 대화가 흘러가버려 원하는 주제 찾기 어렵다"** | 디딤365 |
| 6 | **"토픽 내 공유 파일 검색은 되지만 정작 원하는 파일 찾기 어렵다"** | 디딤365 |
| 7 | **"다양한 기능 한 데 있어 초반 기능 익히는 시간 걸린다"** | 테크뷰 |
| 8 | 공식 가격 비공개 (10인 최소·영업 문의) | flow.team |

#### 💰 요금제 구조

| 요금제 | 내용 |
|---|---|
| **게스트** (무료) | 기능 일부 제한, 프로젝트 초대받아 협업 체험용 |
| **비즈니스 베이직** | **최소 10인** 기준 · 업무 효율·데이터 통제 |
| **비즈니스 프로** | 고급 기능 포함 상위 요금제 |

- 정확한 단가 비공개 (영업 문의)
- **정부지원 혜택 최대 80% 할인** (중소기업)
...
| 네이버웍스 | 한국 대형 |
| 잔디(JANDI) | 한국 중견 |
| **플로우** | 한국 1위급 (48만 팀, 55개국, AI 협업툴 특화) |
| 스윗 | 한국 중견 |
| Notion | 글로벌, 한국 스타트업 |

#### 📚 flow AI 구체 기능 (2026)

- **프로젝트 자동 생성**: 사용자가 목적·기획 문서 입력 → AI가 전체 구조·일정·업무 리스트 자동 구성
- **Gantt 차트 자동 생성**
- **다음 실행 과제 제안** (AI Agent)
- **복수 AI 모델 전환 가능** (ChatGPT, Gemini, Claude, Nanobana)
- 광고 문구: **"직원 100명이 넘기전에 협업툴 + 여러 AI 하나로 합치세요"**

#### 🔍 세션 03가 관찰한 사실 (KNK 맥락)

**흡수할 패턴**:
- **"업무명 + 상태 + Gantt" 통합 뷰** — 진행률 대시보드 UX 참고
- **복수 AI 모델 통합 UX** — KNK도 Claude 외 다른 모델 선택지 추가 가능
- **AI 프로젝트 자동 생성** — 수주별 프로젝트 구조를 AI가 설계하는 개념
- **국내 환경 최적화** — 한국어·한국 기업 관행 반영

**피할 함정**:
- "기능 많아 안정성 나쁨" (사용자 다수 언급) — KNK는 설문 기반 필수 기능만 집중
- **이메일 기능 없음** — KNK는 하이웍스 메일 유지 정책이라 오히려 정합
- 파일 검색 어려움 → KNK는 RAG·pgvector로 해결 가능
- 공식 가격 비공개 = 영업 협상 불투명

**KNK 규모 관찰**:
- 플로우 광고 타겟: **"직원 100명 넘기 전"** → KNK 본사 75명 해당 규모
- 최소 10인 이상 = KNK 자격 충족
- 단, KNK는 이미 HAIST_WORKS 자체 구축 중이라 **중복 도입 위험**

**중요 관찰 (사실)**:
- 플로우 = "메신저 + 프로젝트 + AI 에이전트" 통합 제품
- KNK가 구축 중인 HAIST_WORKS = "FastAPI + 설문 기반 커스텀 + Claude MCP" 통합
- 두 방향의 차이: **플로우는 범용 SaaS · HAIST_WORKS는 KNK 맞춤**
- 플로우 검토 가치 있는 영역: **AI 프로젝트 자동 생성** 기능 아이디어 (벤치마크만)
