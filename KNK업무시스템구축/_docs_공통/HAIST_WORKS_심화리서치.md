...
**KNK 시사점**:
- **Copilot 라이선스 직접 도입은 비싸다** ($30/user/월 × 135명 = 월 4,200$)
...
| DB | **SQLite → PostgreSQL 전환** | 135명 + 관리코드 450 → 멀티 사용자 쓰기 발생 시 PG 필요 |
...
- **DB 마이그레이션**: SQLite → PostgreSQL 전환 계획 필수 (135명 동시 쓰기 한계)
