#!/bin/bash
# ============================================================
# KNK 메신저 — 자동 배포 1회 설정 (auto_deploy.sh 를 cron 매분 등록)
# 실행(NAS, git clone 된 /opt/knk_messenger 안에서): bash deploy/setup_auto_deploy.sh
# 선행 전제는 deploy/auto_deploy.sh 상단 주석(P1~P3) 참고.
# ============================================================
set +e
APP_DIR=/opt/knk_messenger
SERVICE=knk-messenger
...
( crontab -l 2>/dev/null | grep -v 'knk_messenger/deploy/auto_deploy.sh' ; echo "$LINE" ) | crontab -
