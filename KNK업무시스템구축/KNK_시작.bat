@echo off
REM ============================================================
REM   LAST UPDATE: 2026-05-06 v5H171 통화 데이터 end-to-end 연결 수정 — (1)VND 묵음 손실 버그 fix, (2)fx_rate·amount_krw 프로젝트 저장 누락 fix, (3)confirm_order() 단일 SO 통화 상속 누락 fix, (4)project_form 헤더 통화 6종 확장 + 호기 라인 자동 동기화. 수주관리 '+새 수주' 버튼 제거(SO는 프로젝트 진행중 전환 시 자동 발행 구조라 수동 생성 불필요). 임박납기 박스 높이를 달력 그리드 실제 높이에 정확히 맞춤(JS sync). 달력은 자연 높이 유지(과대 stretch 제거), 리스트는 내부 스크롤. v5H168 한달이동(±1개월) + 60건 확장 유지. 수주관리 페이지 전면 재설계 — 4탭(T 검사기/M 자동화/K 기타/소모품) + KPI 6카드 + 상태 파이프라인(수평 칸반) + 출하 캘린더(이번달+다음달) + 강화 목록표(D-day 뱃지·진행률 바·납기색상행). /sales/orders 라우트 매개변수 추가(tab/status/period/currency/q/sort/due_date). 소모품 탭은 consumable_orders 테이블 별도 조회. 신규 partial 2종(_v5_partials/so_pipeline.html, so_calendar.html). 백워드 호환 PRAGMA 컬럼 동적 감지 유지. 정렬 기본 = 납기 임박순(오버듀→임박→여유). 캘린더 색상: 오버듀 적색/D-3 주황/D-7 노랑/여유 녹색. 권한: can_use_sales (_s1_guard).
