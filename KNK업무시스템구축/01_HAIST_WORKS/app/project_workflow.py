...
# ═══════════════════════════════════════════════════════════════════════════════
#  v5H226z824 (대표 지시) — 수주번호 병합도구 (과다발번 정리)
#
#  배경: 예전 일괄(엑셀) 업로드가 같은 관리번호+발주일+납기+납품지+통화+담당자인데
#        단가만 다른 장비(호기) 행을 '행마다 새 수주번호'로 발번 → 한 프로젝트에
#        같은 속성의 수주번호가 2개 이상 존재(예: 011T2606 = T-260615-1 / -4).
#        z820으로 '앞으로의 업로드'는 자동으로 한 SO에 호기별 단가로 묶이지만,
#        기존 과다발번분은 자동으로 합쳐지지 않는다. 이 도구가 관리자 미리보기 후
#        한 수주번호(+호기 N개)로 병합한다.
#
#  ⚠ 재무 데이터를 옮기고 지우는 민감 작업. 반드시:
#     - 미리보기(GET)는 조회만(DB 무변경)
#     - 실제 병합(POST)은 관리자가 안전 그룹을 명시 선택+확인했을 때만
#     - 세금계산서 발행/출하·수금(SHIPPED/INVOICED/PAID/READY_TO_SHIP) 걸린 그룹은
#       자동병합 제외('수동 확인 필요') — GET/POST 양쪽에서 재확인(이중가드)
# ═══════════════════════════════════════════════════════════════════════════════

# 병합을 막는(=재무문서 엮인) status 값. 이 status인 SO가 그룹에 하나라도 있으면 수동확인.
SO_MERGE_LOCKED_STATUSES = ("READY_TO_SHIP", "SHIPPED", "INVOICED", "PAID")
# 병합 후보에서 제외하는 status (취소). CANCELLED SO는 아예 후보 그룹 구성에서 뺀다.
SO_MERGE_EXCLUDE_STATUSES = ("CANCELLED",)
# 삭제 대상 SO를 참조하면 그 그룹을 스킵하는 표 목록 (order_id 로 orders 를 가리키는 표).
#   order_items 는 keeper 로 재배정하므로 제외. order_status_history 는 병합 후 정리(이관)하므로 제외.
SO_MERGE_BLOCKING_REFS = (
    ("shipments", "order_id"),
    ("receipts_payment", "order_id"),
    ("invoices", "order_id"),
    ("production_orders", "order_id"),
    ("export_orders", "order_id"),
    ("project_items", "order_id"),
)


def _so_order_no_suffix(order_no: str) -> int:
    """수주번호 접미 -N 값. 접미 없음(T-260615)=0, T-260615-4=4. 병합 keeper 판정용(작을수록 keeper)."""
    if not order_no:
        return 10 ** 9   # 번호 없는 건 뒤로
...
def _so_has_tax_invoice(c, order_id: int, order_no: str, project_id, item_ids: list[int]) -> bool:
    """이 SO가 세금계산서에 엮였는지 — 두 소스 모두 확인.
       ① orders.tax_invoice_issued=1 (또는 tax_invoice_no 존재)
       ② tax_invoice_lines: 이 프로젝트(ref_id)의 이 SO 호기(unit_iids) 가 발행분('발행')에 포함.
    """
    # ① orders 컬럼 플래그
    try:
        if _col_exists(c, "orders", "tax_invoice_issued"):
            r = c.execute("SELECT tax_invoice_issued, "
                          + ("tax_invoice_no" if _col_exists(c, "orders", "tax_invoice_no") else "NULL")
                          + " FROM orders WHERE id=?", (order_id,)).fetchone()
            if r and (int(r[0] or 0) == 1 or (r[1] or "").strip()):
                return True
    except Exception:
        pass
    # ② tax_invoice_lines (묶어발행 z488) — 이 SO의 호기 id 가 발행된 라인에 들어있으면 잠금
    try:
        if _table_exists(c, "tax_invoice_lines") and item_ids:
            # 발행(취소 아님) 상태의 라인만 — tax_invoices.status 조인
            q = (
                "SELECT l.unit_iids FROM tax_invoice_lines l "
                "JOIN tax_invoices t ON t.id = l.ti_id "
                "WHERE COALESCE(t.status,'발행') <> '취소' "
                "AND (l.ref_id=? OR l.mgmt_code IS NOT NULL)"
            )
            want = set(item_ids)
            for row in c.execute(q, (project_id,)).fetchall():
                raw = (row[0] or "").strip()
                if not raw:
                    continue
                got = set()
...
def find_so_merge_groups(c) -> list[dict]:
    """[미리보기·조회 전용 — DB 무변경] 병합 후보 그룹을 찾아 반환.

    그룹 키 = (project_id, order_date[:10], due_date, ship_to, currency, cc_name).
    같은 그룹에 서로 다른 order_no 가 2개 이상이고, 장비(so_type EQUIPMENT/NULL)·
    status<>CANCELLED 인 그룹만 후보.

    각 그룹에 safety 플래그:
      - locked=True (수동 확인 필요·체크 불가): 그룹의 어느 SO라도 세금계산서 발행 또는
        status ∈ SO_MERGE_LOCKED_STATUSES, 또는 삭제대상이 재무표(shipments 등)를 참조.
      - locked=False: 안전 → 체크박스 선택 가능.

    Returns: [{ project_id, mgmt_code, project_name, keeper_order_no, locked, lock_reasons,
                total_units, orders:[{id, order_no, unit_qty, item_count, amount, status,
                                       is_keeper, tax, refs}] }]
    """
    has_sotype = _col_exists(c, "orders", "so_type")
    has_ccname = _col_exists(c, "orders", "cc_name")
    has_ship   = _col_exists(c, "orders", "ship_to")
    has_cur    = _col_exists(c, "orders", "currency")
    has_uqty   = _col_exists(c, "orders", "unit_qty")

    sel = [
        "o.id", "o.order_no", "o.project_id", "o.total_amount", "o.status",
        "substr(COALESCE(o.order_date,''),1,10) AS od",
        "COALESCE(o.due_date,'') AS dd",
        ("COALESCE(o.ship_to,'')" if has_ship else "''") + " AS st",
        ("COALESCE(o.currency,'KRW')" if has_cur else "'KRW'") + " AS cur",
        ("COALESCE(o.cc_name,'')" if has_ccname else "''") + " AS cc",
        ("COALESCE(o.unit_qty,1)" if has_uqty else "1") + " AS uq",
        ("COALESCE(o.so_type,'EQUIPMENT')" if has_sotype else "'EQUIPMENT'") + " AS sot",
    ]
    where = "o.project_id IS NOT NULL"
    if has_sotype:
        # 장비만: EQUIPMENT 또는 NULL(구데이터 호환). 비장비(CONSUMABLE/SERVICE/OTHER)는 제외.
        where += " AND (o.so_type IS NULL OR o.so_type='EQUIPMENT')"
    rows = [dict(r) for r in c.execute(
        f"SELECT {', '.join(sel)} FROM orders o WHERE {where} ORDER BY o.project_id, o.id"
    ).fetchall()]

    # 그룹핑
    groups: dict[tuple, list[dict]] = {}
    for r in rows:
        if (r.get("status") or "") in SO_MERGE_EXCLUDE_STATUSES:
            continue   # 취소 SO는 그룹 구성에서 제외
        key = (r["project_id"], r["od"], r["dd"], r["st"], r["cur"], r["cc"])
        groups.setdefault(key, []).append(r)

    out = []
    for key, so_list in groups.items():
        # 서로 다른 order_no 2개 이상만 후보
        distinct_nos = {(s.get("order_no") or "") for s in so_list}
        if len(so_list) < 2 or len(distinct_nos) < 2:
            continue

        project_id = key[0]
        # 프로젝트 메타
        pr = c.execute(
            "SELECT mgmt_code, COALESCE(name,'') AS pname FROM projects WHERE id=?",
            (project_id,)
        ).fetchone() if _table_exists(c, "projects") else None
        mgmt_code = (pr["mgmt_code"] if pr and "mgmt_code" in pr.keys() else "") or ""
        pname = (pr["pname"] if pr and "pname" in pr.keys() else "") or ""

        # keeper = order_no 접미 -N 최소(동률이면 id 최소)
        so_list_sorted = sorted(so_list, key=lambda s: (_so_order_no_suffix(s.get("order_no") or ""), s["id"]))
        keeper = so_list_sorted[0]

        lock_reasons = []
        so_view = []
        for s in so_list_sorted:
            # 이 SO의 order_items id/금액
            items = c.execute(
                "SELECT id, COALESCE(amount,0) FROM order_items WHERE order_id=?", (s["id"],)
            ).fetchall()
            item_ids = [int(x[0]) for x in items]
            item_sum = sum(float(x[1] or 0) for x in items)
            # 세금계산서 엮임?
            tax = _so_has_tax_invoice(c, s["id"], s.get("order_no") or "", project_id, item_ids)
            if tax:
                lock_reasons.append(f"{s.get('order_no') or s['id']}: 세금계산서 발행됨")
            # 잠금 status?
            if (s.get("status") or "") in SO_MERGE_LOCKED_STATUSES:
                lock_reasons.append(f"{s.get('order_no') or s['id']}: 상태 {s.get('status')}")
            # 삭제대상(=keeper 아닌 것)이 재무표 참조?
            refs = []
            if s["id"] != keeper["id"]:
                for tbl, colname in SO_MERGE_BLOCKING_REFS:
                    if not _table_exists(c, tbl):
                        continue
                    try:
                        n = c.execute(f"SELECT COUNT(*) FROM {tbl} WHERE {colname}=?", (s["id"],)).fetchone()[0]
                    except Exception:
                        n = 0
                    if n:
                        refs.append(f"{tbl}({n})")
                if refs:
                    lock_reasons.append(f"{s.get('order_no') or s['id']}: 참조 {', '.join(refs)}")
            so_view.append({
                "id": s["id"],
                "order_no": s.get("order_no") or "",
                "unit_qty": int(s.get("uq") or 0),
                "item_count": len(item_ids),
                "amount": item_sum,
                "status": s.get("status") or "",
                "is_keeper": s["id"] == keeper["id"],
                "tax": tax,
                "refs": refs,
            })

        # 체크박스 폼 키(POST 파싱과 1:1) — 그룹키 5필드를 파이프로 잇고 urlsafe base64.
        import base64 as _b64
        _raw = "|".join([key[1] or "", key[2] or "", key[3] or "", key[4] or "", key[5] or ""])
        _gkey_b64 = _b64.urlsafe_b64encode(_raw.encode("utf-8")).decode("ascii")
        ...
        out.append({
            ...
            "cc_name": key[5],
            "keeper_id": keeper["id"],
            "keeper_order_no": keeper.get("order_no") or "",
            "locked": len(lock_reasons) > 0,
            "lock_reasons": lock_reasons,
            "total_units": sum(v["item_count"] for v in so_view),
            "group_key_b64": _gkey_b64,
            "orders": so_view,
        })

    # 표시 정렬: 잠금 아닌(병합 가능) 그룹 먼저, 그다음 관리번호
    out.sort(key=lambda g: (g["locked"], g["mgmt_code"], g["project_id"]))
    return out


def merge_so_group(c, project_id: int, order_date: str, due_date: str,
                   ship_to: str, currency: str, cc_name: str,
                   changed_by: int = 0) -> dict:
    """[실제 병합·트랜잭션] 한 후보 그룹(그룹키로 지정)을 keeper 하나로 병합.

    ⚠ 호출자(라우트)는 이 함수를 db_session() 트랜잭션 안에서 그룹마다 호출.
       내부에서 세금계산서/status/참조를 재확인(이중가드) — 위배 시 아무것도 바꾸지 않고 skip 반환.

    절차:
      1. 그룹키로 이 프로젝트의 해당 SO들을 다시 조회(장비·비취소).
      2. keeper = order_no 접미 최소(동률 id 최소).
      3. 안전 재확인(세금계산서·status·재무표 참조) — 위배면 skip.
      4. 나머지 SO들의 order_items 를 keeper 로 재배정(order_id 변경).
      5. order_status_history 를 keeper 로 이관(참조 정합).
      6. 비워진 나머지 SO 삭제.
      7. keeper 재계산: total_amount=SUM(items.amount), unit_qty=호기수,
         unit_label = ^\\d+호기$ 항목 id순 1·2·… 재번호(비표준 라벨 보존).
      8. 상태 이력 '수주번호 병합(관리자)' 1줄.

    Returns: {ok, skipped, reason, keeper_id, keeper_order_no, merged_so, moved_units}
    """
    has_sotype = _col_exists(c, "orders", "so_type")
    has_ship   = _col_exists(c, "orders", "ship_to")
    has_cur    = _col_exists(c, "orders", "currency")
    ...
    # 1. 그룹키로 SO 재조회 (GET과 동일 그룹 정의)
    conds = ["o.project_id=?", "substr(COALESCE(o.order_date,''),1,10)=?", "COALESCE(o.due_date,'')=?"]
    params = [project_id, order_date or "", due_date or ""]
    conds.append(("COALESCE(o.ship_to,'')" if has_ship else "''") + "=?")
    params.append(ship_to or "")
    conds.append(("COALESCE(o.currency,'KRW')" if has_cur else "'KRW'") + "=?")
    params.append(currency or "KRW")
    conds.append(("COALESCE(o.cc_name,'')" if has_ccname else "''") + "=?")
    params.append(cc_name or "")
    if has_sotype:
        conds.append("(o.so_type IS NULL OR o.so_type='EQUIPMENT')")
    rows = [dict(r) for r in c.execute(
        "SELECT o.id, o.order_no, o.total_amount, o.status FROM orders o WHERE "
        + " AND ".join(conds) + " ORDER BY o.id", params
    ).fetchall()]
    # 취소 SO 제외
    rows = [r for r in rows if (r.get("status") or "") not in SO_MERGE_EXCLUDE_STATUSES]

    distinct_nos = {(r.get("order_no") or "") for r in rows}
    if len(rows) < 2 or len(distinct_nos) < 2:
        return {"ok": False, "skipped": True, "reason": "병합 대상 아님(그룹 변동)", "merged_so": 0, "moved_units": 0}

    # 2. keeper
    rows_sorted = sorted(rows, key=lambda r: (_so_order_no_suffix(r.get("order_no") or ""), r["id"]))
    keeper = rows_sorted[0]
    others = rows_sorted[1:]

    # 3. 안전 재확인 (이중가드)
    for r in rows_sorted:
        items = c.execute("SELECT id FROM order_items WHERE order_id=?", (r["id"],)).fetchall()
        item_ids = [int(x[0]) for x in items]
        if _so_has_tax_invoice(c, r["id"], r.get("order_no") or "", project_id, item_ids):
            return {"ok": False, "skipped": True,
                    "reason": f"세금계산서 발행분 포함({r.get('order_no')}) — 자동병합 제외",
                    "merged_so": 0, "moved_units": 0}
        if (r.get("status") or "") in SO_MERGE_LOCKED_STATUSES:
            return {"ok": False, "skipped": True,
                    "reason": f"출하/수금 상태 포함({r.get('order_no')}={r.get('status')}) — 자동병합 제외",
                    "merged_so": 0, "moved_units": 0}
    # 삭제 대상(others)이 재무표 참조하면 스킵
    for r in others:
        for tbl, colname in SO_MERGE_BLOCKING_REFS:
            if not _table_exists(c, tbl):
                continue
            try:
                n = c.execute(f"SELECT COUNT(*) FROM {tbl} WHERE {colname}=?", (r["id"],)).fetchone()[0]
            except Exception:
                n = 0
            if n:
                return {"ok": False, "skipped": True,
                        "reason": f"{r.get('order_no')} 가 {tbl}({n}) 참조 — 안전상 스킵",
                        "merged_so": 0, "moved_units": 0}

    keeper_id = keeper["id"]
    moved_units = 0
    # 4. 나머지 SO의 order_items → keeper 재배정
    for r in others:
        cur = c.execute("UPDATE order_items SET order_id=? WHERE order_id=?", (keeper_id, r["id"]))
        moved_units += cur.rowcount if cur.rowcount and cur.rowcount > 0 else 0
    # 5. order_status_history 이관(있으면)
    if _table_exists(c, "order_status_history"):
        for r in others:
            try:
                c.execute("UPDATE order_status_history SET order_id=? WHERE order_id=?", (keeper_id, r["id"]))
            except Exception:
                pass
    # 6. 비워진 나머지 SO 삭제
    deleted = 0
    for r in others:
        c.execute("DELETE FROM orders WHERE id=?", (r["id"],))
        deleted += 1

    # 7. keeper 재계산 + 호기 재번호
    _recalc_keeper_units(c, keeper_id)

    # 8. 상태 이력
    if _table_exists(c, "order_status_history"):
        try:
            merged_list = ", ".join((r.get("order_no") or str(r["id"])) for r in others)
            c.execute(
                "INSERT INTO order_status_history(order_id, from_status, to_status, changed_by, note) "
                "VALUES(?,?,?,?,?)",
                (keeper_id, keeper.get("status") or "CONFIRMED", keeper.get("status") or "CONFIRMED",
                 changed_by or None,
                 f"수주번호 병합(관리자) — {merged_list} → {keeper.get('order_no')} (호기 {moved_units}줄 이관·호기 재번호)")
            )
        except Exception:
            pass

    return {"ok": True, "skipped": False, "reason": "",
            "keeper_id": keeper_id, "keeper_order_no": keeper.get("order_no") or "",
            "merged_so": deleted, "moved_units": moved_units}


def _recalc_keeper_units(c, keeper_id: int) -> None:
    """keeper SO의 호기 재번호 + 헤더 재계산.
       - order_items 중 unit_label 이 '^\\d+호기$' 인 항목만 id순 1호기·2호기·… 재부여(비표준 라벨 보존).
       - orders.total_amount = SUM(order_items.amount), unit_qty = 호기(라인) 수,
         unit_label = keeper 호기 라벨을 ' · ' 로 재조립(append_unit_to_order 표기와 일치).
    """
    items = [dict(r) for r in c.execute(
        "SELECT id, COALESCE(unit_label,'') AS ul, COALESCE(amount,0) AS amt "
        "FROM order_items WHERE order_id=? ORDER BY id", (keeper_id,)
    ).fetchall()]
    _std = re.compile(r"^\d+호기$")
    n = 0
    labels_for_header = []
    for it in items:
        if _std.match((it["ul"] or "").strip()):
            n += 1
            new_lbl = f"{n}호기"
            if new_lbl != it["ul"]:
                c.execute("UPDATE order_items SET unit_label=? WHERE id=?", (new_lbl, it["id"]))
            labels_for_header.append(new_lbl)
        else:
            # 비표준 라벨은 보존, 헤더 표기에는 원문 사용(비면 스킵)
            if (it["ul"] or "").strip():
                labels_for_header.append(it["ul"].strip())
    total = sum(float(it["amt"] or 0) for it in items)
    unit_qty = len(items)
    header_label = " · ".join(labels_for_header) if labels_for_header else None
    # 컬럼 유무 방어
    sets, vals = ["total_amount=?"], [total]
    if _col_exists(c, "orders", "unit_qty"):
        sets.append("unit_qty=?"); vals.append(unit_qty)
    if _col_exists(c, "orders", "unit_label"):
        sets.append("unit_label=?"); vals.append(header_label)
