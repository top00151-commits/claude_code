...
WP01_LOCK_SWITCHES = (
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # [대표 지시 2026-07-26 · `CHATGPT_WP03_확산승인철회_및_자재구매센터_본업복귀_지시서_V1`]
    #   호기 확산 승인이 철회되면서, 과거 데이터 정비 **전용** 화면인
    #   `/project/{id}/units/status-sync` 의 **실행**을 잠근다.
    #   ⭐ 잠그는 것은 '과거 데이터 보정' 하나뿐이다. 기술영업팀이 신규 프로젝트의
    #      호기를 등록하는 정상 업무(수주내역 후보 반영)는 **잠그지 않는다**(대표 지시 1항).
    #   ⛔ 다시 열려면 대표 승인 + 이 환경변수 설정이 함께 필요하다(작업 후 재잠금).
    "KNK_ENABLE_UNIT_STATUS_SYNC",      # 과거 데이터 보정(작업일정표 상태 → 호기 신원·진행상태)
    # ─────────────────────────────────────────────────────────────────────────
    # [대표 조건부 승인 2026-07-29 1829 · `CHATGPT_WP04_BOM검증_A단계_중간판정_및_B단계_분리요청`]
    #   BOM 엑셀을 올려 보드에 반영하는 것은 아직 **임시 저장**이다. BOM Release·구매요청이
    #   생기기 전(B단계)까지 이 상태로 구매팀에게 통보하면, 검증 불일치·중복 시트가 있는
    #   자료를 구매 착수 신호로 오해하게 된다.
    #   ⭐ 기능을 **지우지 않고 잠근다**. 원래 이 통보는 z984 대표 지시("등록한 걸 보고
    #      구매팀 업무가 시작")로 만들어진 것이므로, 코드를 남겨 두고 Release·구매요청
    #      제출 조건이 확정된 뒤 그 시점으로 옮겨 되살린다.
    #   ⛔ 잠긴 동안 업로드·임시저장 단계의 실제 통보는 **0건**이어야 한다.
    ...
    with db_session() as c:
        ...
        try:
            ...
            for col, decl in [
                ...
                ("fx_rate",          "ALTER TABLE order_items ADD COLUMN fx_rate REAL"),
