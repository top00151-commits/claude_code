...
def normalize_sheet(ws, spec, log=None):
    ...
    # 맨 마지막: 데이터 이후 빈 행 삭제 (Ctrl+End가 마지막 데이터 행으로 이동)
    if spec.get("trim_empty_rows", True):
        trim_empty_row_formatting(ws, mc, row_start=5, row_end=row_end)
