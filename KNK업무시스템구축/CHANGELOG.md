...
## 🛠 v5H226z452 (2026-06-15) — 휴대폰 사이드바 드로어 자동 닫힘

- 증상: 휴대폰에서 ☰로 메뉴를 연 뒤 항목을 골라도 드로어가 본문 위에 떠 있어 화면을 못 봄.
- 원인: 인앱 링크가 SPA 전환(`spa_nav`, `.main`만 교체·새로고침 없음)이라 `mob-sb-open`이 안 풀림.
- 수정: `_v5_partials/chrome.html` — 메뉴 항목 탭 / 바깥(어둠막) 탭 / SPA 전환(`knk:navigated`) / ESC 시 드로어 닫힘. 데스크톱(≥1024px) 영향 0.
- 기록: `작업기록/작업기록_2026-06-15_휴대폰_사이드바_드로어_자동닫힘_z452.md`, `99_DISPATCH/2026-06-15_FYI_01_…_z452.md`
...
- chooser 페이지 하단에 회색 박스로 [📊 프로젝트 목록] [📦 소모품 발주 목록] 진입 링크 추가 (목록 접근 보존)
