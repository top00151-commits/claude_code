#!/bin/bash
# z1123 회귀 묶음 — 고친 사본(z1123/app_new)으로 기존 시험 전부 + z1122 삭제(서버 33·화면 46) + z1123 새 시험(서버·화면)
...
W="$B/2026-09-22_0022_이음회의삭제도_회의록함께_WORKS준비물"
R="$SP/z1123/reg"; APPW="C:/Users/top00/AppData/Local/Temp/claude/C--Users-top00-JR-Claude---/31b97fce-95eb-4c01-8a66-eddd91b2bd12/scratchpad/z1123/app_new"
APPU="$SP/z1123/app_new"
...
timeout 900 "$P312" "$W19/ui_test_cards_z1108.py" > "$R/ui_test_cards.log" 2>&1; res ui_test_cards_z1108 "$R/ui_test_cards.log"
kill_port 8921; kill_port 8922; rm -f "$W19/seed.json"
# 7) z1122 삭제 화면 46 · z1123 화면(취소 알림 안내) — 서버를 새로 띄워 각각
"$W/restart23.sh" app_new 8942 >> "$OUT" 2>&1 && { timeout 900 "$P312" "$B/2026-09-21_2313_회의록삭제_두번확인_이음회의연동_WORKS준비물/ui_del.py" "$W/seed_app_new" > "$R/ui_del.log" 2>&1; res ui_del "$R/ui_del.log"; }
"$W/restart23.sh" app_new 8942 >> "$OUT" 2>&1 && { timeout 900 "$P312" "$W/ui_z1123.py" "$W/seed_app_new" > "$R/ui_z1123.log" 2>&1; res ui_z1123 "$R/ui_z1123.log"; }
kill_port 8942
# 8) 서버 시험(TestClient)
