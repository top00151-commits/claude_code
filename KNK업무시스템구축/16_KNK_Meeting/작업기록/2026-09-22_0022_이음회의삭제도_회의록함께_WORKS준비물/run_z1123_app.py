# -*- coding: utf-8 -*-
"""z1123 화면 시험용 WORKS 서버 (기본 127.0.0.1:8942 · z1122 run_del_app + D8 = 이음이 취소 알림 3명) — 가짜 이음(삭제 입구·모아보기 목록).
...
APP = sys.argv[1]
PORT = int(sys.argv[2]) if len(sys.argv) > 2 else 8942
os.chdir(APP)
...
PLAN = {9401: "ok", 9402: "not_allowed", 9403: "html404", 9404: "conn", 9405: "ok", 9406: "ok", 9407: "ok_n3"}
...
class _FakeHttpx:
    def post(self, url, headers=None, json=None, timeout=None):
        if url.endswith("/api/works/meetings/delete"):
            ...
            if k == "ok_n3":      # z1123: 시작 전 회의 — 이음이 참석자 3명에게 취소 알림을 보냈다고 답함
                DELETED.add(int(j["msg_meeting_id"]))
                return _R(200, {"ok": True, "notified": 3})
            if k == "not_allowed":
...
with db_session() as c:
    ...
    ids = {
        ...
        "D8": mtg("D8 시작 전 · 취소 알림 3명", owner, 9407, owner),
