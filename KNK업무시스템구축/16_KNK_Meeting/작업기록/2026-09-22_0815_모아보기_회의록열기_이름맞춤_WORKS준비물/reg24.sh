#!/bin/bash
# z1124 회귀 묶음 — 고친 사본(z1124/app_new)으로 기존 시험 전부(모아보기 원래 화면·삭제 화면은 새 이름을 기대하는 z1124 사본)
...
W="$B/2026-09-22_0815_모아보기_회의록열기_이름맞춤_WORKS준비물"; W23="$B/2026-09-22_0022_이음회의삭제도_회의록함께_WORKS준비물"
R="$SP/z1124/reg"; APPW="C:/Users/top00/AppData/Local/Temp/claude/C--Users-top00-JR-Claude---/31b97fce-95eb-4c01-8a66-eddd91b2bd12/scratchpad/z1124/app_new"
APPU="$SP/z1124/app_new"
...
timeout 900 "$P312" "$W/ui_test_cards_z1124.py" > "$R/ui_test_cards.log" 2>&1; res ui_test_cards_z1124 "$R/ui_test_cards.log"
kill_port 8921; kill_port 8922; rm -f "$W19/seed.json"
# 7) z1122 삭제 화면 46 · z1123 화면(취소 알림 안내) — 서버를 새로 띄워 각각
"$W/restart24.sh" app_new 8942 >> "$OUT" 2>&1 && { timeout 900 "$P312" "$W/ui_del_z1124.py" "$W/seed_app_new" > "$R/ui_del.log" 2>&1; res ui_del_z1124 "$R/ui_del.log"; }
"$W/restart24.sh" app_new 8942 >> "$OUT" 2>&1 && { timeout 900 "$P312" "$W23/ui_z1123.py" "$W/seed_app_new" > "$R/ui_z1123.log" 2>&1; res ui_z1123 "$R/ui_z1123.log"; }
kill_port 8942
# 8) 서버 시험(TestClient)
for t in "2026-09-18_1016_회의록_녹음서버저장_창닫혀도끝내기_WORKS준비물/verify_rec.py" "2026-09-20_2240_회의록_녹음기공유로자동정리_WORKS준비물/verify_share.py" "2026-09-18_2249_회의록_녹음중뒤로가기_다른앱경고_WORKS준비물/verify_msg_rec.py" "2026-09-21_1830_회의카드모아보기_끝났는데회의중_WORKS준비물/verify_cards_rec.py" "2026-09-21_2200_이음카드_이어서녹음_rec_mine_WORKS준비물/verify_msg_mine.py" "2026-09-17_0845_회의카드모아보기_WORKS준비물/verify_cards.py" "2026-09-21_2313_회의록삭제_두번확인_이음회의연동_WORKS준비물/verify_del.py" "2026-09-22_0022_이음회의삭제도_회의록함께_WORKS준비물/verify_z1123.py"; do
