#!/bin/bash
# z1122 회귀 묶음 — 고친 사본(z1122/app_new)으로 녹음·모아보기·나갈 때 알림 등 기존 시험 전부
...
R="$SP/z1122/reg"; APPW="C:/Users/top00/AppData/Local/Temp/claude/C--Users-top00-JR-Claude---/31b97fce-95eb-4c01-8a66-eddd91b2bd12/scratchpad/z1122/app_new"
APPU="$SP/z1122/app_new"
...
kill_port(){ /tmp/killport.sh "$1" || { echo "[포트 $1] 비우지 못함 — 시험 중지" >> "$OUT"; return 1; }; }
...
res(){ local name="$1" log="$2"; echo "== $name: $(grep -E '합계|통과 [0-9]+ [·/] 실패|=====.*통과|결과: [0-9]+/[0-9]+' "$log" | tail -1)" >> "$OUT"; grep -E "^\s*(FAIL|\[FAIL\])|❌|Traceback" "$log" | head -6 >> "$OUT"; }
...
kill_port 8936
# 3) 마이크 안내 · 4) 나갈 때 알림 · 5) 창 닫힌 녹음 맨 위(z1120 · ⑫ 안내 글만 새 글로)
